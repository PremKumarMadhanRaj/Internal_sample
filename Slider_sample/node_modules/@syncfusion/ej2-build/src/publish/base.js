...
gulp.task('publish-blazor-source', function (done) {
    publishScripts('ej2-blazor-source', './blazor-source', 'Scripts', 'Scripts', true, done);
});
...
function publishScripts(sampleName, localPath, stylesPath, sourcePath, isBlazorSource, done) {
    ...
    if (clone.code !== 0) {
        ...
        var addFiles = 'git add ' + sourcePath + '/ej2.min.js ' + stylesPath + '/*.css';
        shelljs.exec(addFiles);
        var message = 'latest scripts and styles has been shipped';
...
gulp.task('ej2-js', function () {
    var runSequence = require('run-sequence');
    runSequence('ej2-js-pack', 'package-json', 'ej2-write-dts', 'publish-blazor-source', 'publish-blazor-sb-scripts',
